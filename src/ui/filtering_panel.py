...
class FilteringPanel(QWidget):
    ...
    def apply_community_filter(self):
        """Filter graph to show only nodes in selected community"""
        if not self.graph_manager.has_graph():
            return
            
        # Get selected community text
        selected_text = self.comm_combo.currentText()
        if selected_text == "No communities detected":
            return
            
        # Extract community ID from text (format: "Community X (Y nodes)")
        try:
        ...
        # Get communities from graph manager
        communities = self.graph_manager.get_communities()
        if not communities:
            return
            
        # Filter nodes in selected community
    ...
    def apply_degree_filter(self):
        """Filter graph based on node degree range"""
        if not self.graph_manager.has_graph():
            return
    ...
    def reset_filters(self):
        """Reset all filters and display the original graph"""
        if not self.graph_manager.has_graph():
            return
            
        # Reset filtered graph
