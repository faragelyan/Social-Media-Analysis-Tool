...
#         # Centrality table - Added PageRank column
#         self.centrality_table = QTableWidget(0, 6)  # Changed from 5 to 6 columns
...
#             pagerank = nx.pagerank(G)  # Calculate PageRank
...
#                 self.centrality_table.setItem(i, 5, QTableWidgetItem(f"{pagerank[node]:.4f}"))  # Add PageRank
...
# Requirement 4: Graph Metrics and Statistics (degree, clustering, path length, centrality)
# Requirement 9: Link Analysis Techniques (PageRank, Betweenness Centrality)
class MetricsPanel(QWidget):
    def __init__(self, graph_manager):
        super().__init__()
        self.graph_manager = graph_manager
    ...
    def init_ui(self):
        ...
        self.centrality_table = QTableWidget(0, 6)
        self.centrality_table.setHorizontalHeaderLabels([
            "Node", "Degree", "Betweenness", "Closeness", "Eigenvector", "PageRank"
        ])
        self.centrality_table.horizontalHeader().setStretchLastSection(True)
    ...
    def update_metrics(self):
        ...
        try:
            ...
            for i, (node, degree) in enumerate(top_nodes):
                ...
                self.centrality_table.setItem(i, 3, QTableWidgetItem(f"{close_cent[node]:.4f}"))
                self.centrality_table.setItem(i, 4, QTableWidgetItem(f"{eigen_cent[node]:.4f}"))
                self.centrality_table.setItem(i, 5, QTableWidgetItem(f"{pagerank[node]:.4f}"))
